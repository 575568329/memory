...
**跨设备自适应记忆层**：通过 GitHub 私有仓库实现多设备记忆同步

- **仓库地址**：https://github.com/575568329/memory
- **主存储目录**：`C:\Users\fjyu9\.adaptive-memory\`
- **本地链接**：`C:\Users\fjyu9\.claude\projects\C--Users-fjyu9\memory\MEMORY.md`
...
**处理任务时必须优先参考：**
1. 编码风格 → `memory/preferences/coding-style.md`
2. 工作流程 → `memory/preferences/workflow.md`
3. 决策记录 → `memory/decisions/`
...
| 分数范围 | 存储位置 | 示例 |
|---------|---------|------|
| **≥ 7分** | MEMORY.md（核心记忆） | 编程语言偏好、工作流程、核心决策 |
| **4-6分** | 分类文件 | 项目特定配置、偶尔使用的工具 |
| **< 4分** | 归档或删除 | 临时信息、过时记录 |
...
cd ~/.adaptive-memory
git add .
git commit -m "feat: 添加/更新记忆 [类型]"
git push origin main
```

**其他设备同步频率：** 每30分钟自动拉取更新
